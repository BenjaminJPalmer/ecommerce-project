...
fetch(`/ecommerce-project/api/products?limit=3`)
  ...
  .then((products) => {
    if (products.error) {
      ...
      for (const product of products) {
        ...
        cardLink.href = `/ecommerce-project/product?id=${product.id}`;
...
fetch(`/ecommerce-project/api/product-categories?limit=3`)
  .then((response) => response.json())
  .then((categories) => {
    if (categories.error) {
      ...
      for (const category of categories) {
        ...
        const cardLink = document.createElement("a");
        cardLink.href = `/ecommerce-project/category?id=${category.id}`;
        const title = document.createElement("h4");
